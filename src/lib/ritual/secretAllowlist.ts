    ...
    outputs: [
      {
        ...
        components: [
          ...
          { name: "secretOwner", type: "address" },
          { name: "secretsHash", type: "bytes32" },
          { name: "allowlistCid", type: "string" },
          { name: "endpointBase", type: "string" },
...
const allowlistResolvedSelector = toEventSelector(
  "AllowlistCheckResolved(bytes32,address,address,bool,uint16,uint64,uint64)",
);

const allowlistFailedSelector = toEventSelector(
  "AllowlistCheckFailed(bytes32,address,address,string,uint16)",
);

export type ParsedAllowlistEvent =
  | {
      kind: "resolved";
      allowed: boolean;
      httpStatus: number;
...
export function parseAllowlistEvent(
  logs: Log[],
): ParsedAllowlistEvent {
  for (const log of logs) {
    const eventSelector = log.topics[0];

    if (
      eventSelector !== allowlistResolvedSelector &&
      eventSelector !== allowlistFailedSelector
    ) {
      continue;
    }
    ...
    try {
      ...
      if (decoded.eventName === "AllowlistCheckResolved") {
        return {
          kind: "resolved",
          allowed: Boolean(decoded.args.allowed),
          httpStatus: Number(decoded.args.httpStatus),
        };
      }
