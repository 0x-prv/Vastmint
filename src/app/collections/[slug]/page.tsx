...
function NFTCard({
  ...
  return (
    <div className="rounded-2xl border border-[#1a4a2e]/15 bg-[#ede8df] overflow-hidden hover:border-[#1a4a2e]/40 transition-all duration-200 group">
      <Link href={`/nft/${collection.contractAddress}/${tokenId}`}>
        <div className="aspect-square relative overflow-hidden bg-[#e8e3d8]">
          {displayImage ? (
            <img
              src={displayImage}
              alt={displayName}
            ...
            <div className="w-full h-full bg-[#1a4a2e]/10 flex items-center justify-center">
              <span className="text-[#7a9e7a] text-xs font-mono">#{tokenId}</span>
            </div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-[#1a4a2e]/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition" />
          {isListed && (
            <div className="absolute top-2 left-2">
              <span className="rounded-full border border-[#1a4a2e]/30 bg-[#1a4a2e]/10 px-2 py-0.5 text-[10px] font-bold text-[#1a4a2e]">
                Listed
              </span>
            </div>
          )}
          {isMine && (
            <div className="absolute top-2 right-2">
              <span className="rounded-full border border-[#1a4a2e]/20 bg-[#e0dbd0]/80 px-2 py-0.5 text-[10px] font-bold text-[#1a2e1a]">
                Yours
              </span>
            </div>
          )}
      ...
      <div className="p-3">
        <Link href={`/nft/${collection.contractAddress}/${tokenId}`}>
          <p className="text-[#1a2e1a] font-bold text-sm truncate hover:text-[#1a4a2e] transition">
            {displayName}
          </p>
        </Link>
        ...
        {isListed && priceInRitual !== null ? (
          <div className="mt-2">
            ...
            {isMine ? (
              ...
              <button
                ...
                className="mt-2 w-full py-2 rounded-xl bg-[#1a4a2e] hover:bg-[#143d24] disabled:opacity-50 transition text-xs font-bold text-[#f5f0e8] flex items-center justify-center gap-1.5"
              >
                {isBuying && (
          ...
          <p className="text-[#7a9e7a] text-xs mt-1">Not listed</p>
        )}
      </div>
    </div>
  );
...
export default function CollectionPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f5f0e8] text-[#1a2e1a] pb-24">
      {/* Banner */}
      <div className="relative h-48 bg-gradient-to-br from-[#e8e3d8] via-[#e0dbd0] to-[#e0dbd0] overflow-hidden">
        {imageUrl && (
          <img
            src={imageUrl}
            alt={collection.name}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="relative -mt-16 mb-8 flex items-end gap-5 flex-wrap">
          <div className="w-28 h-28 rounded-2xl overflow-hidden border-4 border-[#f5f0e8] bg-[#ede8df] flex-shrink-0">
            {imageUrl ? (
          ...
          <div className="flex gap-3 pb-2">
            <Link
              ...
              className="rounded-xl bg-[#1a4a2e] hover:bg-[#143d24] transition px-5 py-2.5 text-sm font-bold text-[#f5f0e8]"
            >
              Mint NFT
            </Link>
            <a
              ...
              className="rounded-xl border border-[#1a4a2e]/20 hover:bg-[#1a4a2e]/10 transition px-5 py-2.5 text-sm font-bold text-[#4a6741] flex items-center gap-1.5"
            >
              Explorer
        ...
        {collection.description && (
          <p className="text-[#4a6741] text-sm max-w-2xl mb-8 leading-relaxed">
            {collection.description}
          </p>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={label} className="rounded-2xl border border-[#1a4a2e]/15 bg-[#ede8df] px-5 py-4 text-center">
              <p className="text-[#7a9e7a] text-xs">{label}</p>
              <p className={`font-black text-lg mt-1 ${label === "Floor Price" || label === "Mint Price" ? "text-[#1a4a2e]" : "text-[#1a2e1a]"}`}>
                {value}
              </p>
            </div>
          ))}
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-medium transition border ${
                filter === key
                  ...
                  : "border-[#1a4a2e]/20 text-[#4a6741] hover:text-[#1a2e1a] hover:border-[#1a4a2e]/40"
              }`}
            >
              {label}
            </button>
        ...
        {minted === 0 ? (
          <div className="rounded-2xl border border-[#1a4a2e]/20 bg-[#ede8df] p-16 text-center">
            <p className="text-[#4a6741] text-sm">No NFTs minted yet.</p>
            <Link
              href={`/collections/${slug}/mint`}
          ...
          <div className="rounded-2xl border border-[#1a4a2e]/20 bg-[#ede8df] p-16 text-center">
            <p className="text-[#4a6741] text-sm">No listed NFTs found.</p>
            <button
